...
async function _createTable() {
  ...
  for (const [col, def] of [
    ...
    ["approach_alert_sent", "TINYINT(1) NOT NULL DEFAULT 0"],
    ["consecutive_errors",  "INT NOT NULL DEFAULT 0"],
    // initial_total_usd holds the COIN-ONLY equity baseline for coin-margined
    // tokens (BTC/ETH) as of this change — PnL/target tracking is coin-only,
    // matching the user's explicit request. initial_usdc_equity_usd is purely
    // informational (shown in the entry alert alongside the coin figure) and
    // never used in the trigger math.
...
async function _processJob(job) {
  ...
  if (job.status === "active") {
    ...
    // For coin-margined tokens (BTC/ETH), PnL/target tracking is coin-equity
    // ONLY — the USDC side (futures hedge collateral) is excluded from the
    // trigger decision entirely, even though it's still shown for reference.
    // For pure-USDC-margined tokens (SOL_USDC, XRP_USDC, ...) there is no
    // separate coin wallet, so this is identical to the old whole-account
    // behavior. initial_total_usd holds whichever basis applies, captured at
    // job creation — see startAutoClose()/the POST route for the write side.
    const isCoinMargined = col.coin_symbol !== "USDC";
    const currentBasis   = isCoinMargined ? col.coin_equity_usd : col.total_usd;
    ...
    if (!job.approach_alert_sent && targetPnl > 0 && pnl >= targetPnl * APPROACH_THRESHOLD) {
      ...
      await sendTelegramAlert(
        [
          ...
          ``,
          `PnL: +$${pnl.toFixed(2)} / target +$${targetPnl.toFixed(2)} (${((pnl / targetPnl) * 100).toFixed(1)}%)${isCoinMargined ? ` — ${col.coin_symbol} equity only` : ""}`,
    ...
    if (currentBasis >= parseFloat(job.target_total_usd)) {
      await _log(job.id,
        `TARGET HIT — ${col.coin_symbol} equity $${col.coin_equity_usd.toFixed(2)}${isCoinMargined ? " (tracked basis)" : ""} | USDC $${col.usdc_equity.toFixed(2)} (reference only) | PnL +$${pnl.toFixed(2)}`
      );
...
async function _closeOption(job) {
  ...
  const result = await placeLimitClose(base, token, job.opt_instrument, liveSize, job.opt_dir);
...
async function _closeFutures(job) {
  ...
  const result     = await placeMarketClose(base, token, job.fut_instrument, liveFutSize, job.fut_dir);
  const orderId    = result.order?.order_id;
...
async function _finishJob(jobId) {
  try {
    ...
    // Same coin-only basis as the trigger check in _processJob — Net PnL
    // must be measured against the same number that decided to close.
    const isCoinMargined = col ? col.coin_symbol !== "USDC" : job.initial_usdc_equity_usd == null;
    const finalEquity = col
      ? (isCoinMargined ? col.coin_equity_usd : col.total_usd)
