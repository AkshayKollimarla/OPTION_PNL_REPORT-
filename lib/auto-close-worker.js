 ...
 * Polls every 2 s for active jobs, closes positions when equity target is hit.
...
import {
  auth, collateral, positionFlat, livePositionSize, isOptionExpired, placeLimitClose, placeMarketClose, rpc,
  _fetchJsonOrOutage, invalidateAuth, effectiveTick, roundToTick,
...
async function _createTable() {
  ...
  for (const [col, def] of [
    ...
    // initial_usdc_equity_usd is purely informational in 'coin' mode (shown
    // in the entry alert alongside the coin figure), never used in the
    // trigger math.
...
// Skip a fire while the previous tick is still running. setInterval keeps its
// fixed cadence no matter how long an async tick takes, so a slow tick used to
// have the next one start on top of it — and since a job's opt_order_id is
// written only AFTER the order returns, both ticks read NULL, both decided no
// order was working, and both placed one. That put duplicate reduce_only
// orders on the book for the same position in the combo worker; this worker
// has the identical structure. Skipping is safe: the work is idempotent and
// the next fire retries 2s later.
  ...
  for (const job of jobs) {
    try {
      ...
      if (job.consecutive_errors) {
        try { await pool.query(`UPDATE auto_close_jobs SET consecutive_errors=0 WHERE id=?`, [job.id]); } catch {}
      }
    } catch (err) {
      ...
      if (err.isExchangeOutage) {
        ...
        console.error(`[auto-close-worker #${job.id}] Deribit appears to be down (attempt ${nextCount}), not counted toward the failure limit:`, err.message);
        try { await pool.query(`UPDATE auto_close_jobs SET consecutive_errors=? WHERE id=?`, [nextCount, job.id]); } catch {}
        if (nextCount % 30 === 1) {
      ...
      console.error(`[auto-close-worker #${job.id}] error ${nextCount}/${ERROR_THRESHOLD}:`, err.message);
...
async function _processJob(job) {
  ...
  if (job.status === "active") {
    ...
    // job): 'coin' — coin-equity ONLY, the USDC side (futures hedge
    // collateral) is excluded from the trigger entirely, only shown for
    // reference. 'combined' — whole account (coin + USDC summed), same as
    // the original pre-coin-tracking behavior. Confirmed incident driving
    // the 'coin' default: a job hit its USDC-driven target while coin
    // equity was actually negative — a real loss reported as a win.
    // Pure-USDC-margined tokens (SOL_USDC, XRP_USDC, ...) have no separate
    // coin wallet, so both modes are identical for those regardless of
    // job.monitor_mode.
...
// Options always close as a maker at the mark price — never falls back to
// market. If the order sits unfilled, it re-quotes at the current mark price
// every tick (2s), same chase behavior as the entry engine. Futures still
// close at market (accepted — the hedge needs to come off immediately once
// the option leg is done).
...
async function _closeOption(job) {
  ...
  if (job.opt_order_id) {
    ...
    // Still open — re-quote at the current mark price if it has moved (chase, like the entry engine)
    const ticker = (await _fetchJsonOrOutage(`${base}/public/ticker?instrument_name=${encodeURIComponent(job.opt_instrument)}`)).result ?? {};
    ...
    // mark — placeLimitClose snaps to the instrument's tick, so a resting
    // order sits up to a full tick from mark by design. Comparing to the raw
    // mark made that permanent gap exceed the threshold on coarse-tick
    // instruments, so the order was cancelled and re-placed at the identical
    // price every tick, losing queue position and never filling. Same defect
    // and fix as the combo worker.
    const reTick = await effectiveTick(base, job.opt_instrument, markPrice);
    const targetPrice = roundToTick(Math.max(markPrice, reTick), reTick, job.opt_dir);
    if (markPrice > 0 && Math.abs(targetPrice - orderPrice) > OPT_REQUOTE_THRESHOLD) {
      await _log(job.id, `Option mark price moved ${orderPrice.toFixed(5)} → ${markPrice.toFixed(5)}, re-quoting maker order...`);
      try {
  ...
  // REAL live position size, not the stored opt_qty — they can drift apart
  // (partial fill, assignment, manual intervention), and a reduce_only
  // order sized larger than the actual open position gets rejected by
  // Deribit (invalid_reduce_only_order) forever, which is a confirmed
  // incident: a job spent 3+ minutes re-quoting and failing on exactly this.
  const liveSize = await livePositionSize(base, token, job.opt_instrument);
  if (liveSize === null) {
    await _log(job.id, `Could not confirm live position size for ${job.opt_instrument} — skipping this tick, will retry.`);
    return;
  }
  if (liveSize === 0) {
    // Already flat — next tick's positionFlat() check at the top will
    // pick this up and finish the job normally.
    return;
  }

  // exactAmount=true — liveSize already comes back from livePositionSize in
  // Deribit's own "amount" units for this instrument; re-deriving it would
  // double-convert (see the note on livePositionSize).
  const result = await placeLimitClose(base, token, job.opt_instrument, liveSize, job.opt_dir, true);
  const orderId = result.order?.order_id;
  const price   = result.order?.price;
  await _log(job.id, `Option maker close placed: ${liveSize}x ${job.opt_instrument} @ ${price} [order ${orderId}]`);
...
async function _closeFutures(job) {
  ...
  // Same live-size fix as the option close above — reduce_only orders sized
  // off a stale stored qty get rejected if the real position differs.
  const liveFutSize = await livePositionSize(base, token, job.fut_instrument);
  if (liveFutSize === null) {
    await _log(job.id, `Could not confirm live futures position size for ${job.fut_instrument} — skipping this tick, will retry.`);
    return;
  }
  if (liveFutSize === 0) {
    await _log(job.id, `Futures position already flat (${job.fut_instrument}) — nothing left to close.`);
  ...
  const result     = await placeMarketClose(base, token, job.fut_instrument, liveFutSize, job.fut_dir, true);
  const orderId    = result.order?.order_id;
  const closePrice = parseFloat(result.order?.average_price ?? result.order?.price ?? 0);
  await _log(job.id, `Futures market close placed: ${liveFutSize}x ${job.fut_instrument} @ ${closePrice} [order ${orderId}]`);
...
async function _finishJob(jobId) {
  try {
    ...
    // Same basis as the trigger check in _processJob — Net PnL must be
