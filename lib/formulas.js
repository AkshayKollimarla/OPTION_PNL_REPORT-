...
import { DEFAULT_WORKING_HOURS } from "./fields";
...
export const FORMULAS = {
  ...
  // 2. Per Hour RTPS = RTPS / Working Hours
  //
  // The divisor used to be a fixed 24, which assumed the bot ran round the
  // clock. A bot that ran six hours then read a quarter of its true hourly
  // rate. Working Hours defaults to 24, so an entry nobody adjusts computes
  // exactly as before; a blank, zero or negative value falls back to the same
  // default rather than dividing by nothing.
  per_hour_rtps: (f) => {
    const rtps = n(f.rtps);
    if (!rtps) return "";
    const hours = n(f.working_hours);
    return rtps / (hours > 0 ? hours : DEFAULT_WORKING_HOURS);
  },

  // 15. Volume = (RTPS x 2 x Per Step Qty) x Bot Entry Price
  //
  // Sits after rtps because it reads the computed value, not a typed one.
  // Blank until RTPS resolves, so an empty form does not show a confident $0.
  volume: (f) => {
    const rtps = n(f.rtps);
    if (!rtps) return "";
    return rtps * 2 * n(f.per_step_qty) * n(f.bot_entry_price);
  },

  // ── depends on raw inputs only ───────────────────────────────────────────

  // 3. Net-PNL = RTP-PNL + Rebates  (Hedge Recovery PNL and Flatten are shown for
  // reference but not included in Net PNL)
  net_pnl: (f) =>
    n(f.rtp_pnl) + n(f.rebates),
