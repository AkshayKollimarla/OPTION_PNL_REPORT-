// Single source of truth for every field the bot tracks.
// Used by the manual-entry form, the API insert, and the dashboard.
// format: "currency" | "percent" | "number" | "text"

export const METRIC_CARDS = [
  { key: "rtps", label: "RTPS", format: "number", color: "blue" },
  { key: "rtp_pnl", label: "RTP-PNL", format: "currency", color: "green" },
  { key: "per_hour_rtps", label: "Per Hour RTPS", format: "number", color: "purple" },
  { key: "rebates", label: "Rebates", format: "currency", color: "orange" },
  { key: "gamma_booked", label: "Hedge Recovery PNL", format: "currency", color: "indigo" },
  { key: "flatten_pnl", label: "Flatten PNL", format: "currency", color: "teal" },
  { key: "net_pnl", label: "Net-PNL", format: "currency", color: "green" },
  { key: "volume", label: "Volume", format: "compact-currency", color: "blue" },
  { key: "apy", label: "APY", format: "percent", color: "purple" },
];

// Hours the bot ran for an entry. Per Hour RTPS divides by this.
//
// Kept OUT of METRIC_CARDS on purpose. That array is not just the form's
// Performance Metrics section — it also renders the Dashboard's metric cards
// and the Analysis page's metrics, and an input that exists to scale one of
// those figures is not itself a result worth a card of its own. The form
// places it beside Per Hour RTPS explicitly instead.
export const DEFAULT_WORKING_HOURS = 24;
export const WORKING_HOURS_FIELD = {
  key: "working_hours",
  label: "Working Hours",
  format: "number",
  placeholder: String(DEFAULT_WORKING_HOURS),
  hint: "Per Hour RTPS = RTPS ÷ working hours",
};

export const BOT_DETAILS_LEFT = [
  { key: "investment", label: "Investment", format: "currency" },
  { key: "entry_futures", label: "Entry Futures", format: "currency" },
  { key: "entry_futures_price", label: "Entry Futures Price", format: "currency" },
  { key: "bot_entry_price", label: "Bot Entry Price", format: "currency" },
  { key: "market_making_qty", label: "Market Making Qty", format: "number" },
  { key: "average_spread", label: "Average Spread", format: "currency" },
  { key: "target_spread", label: "Target Spread", format: "currency" },
  { key: "basket_distance", label: "Basket Distance", format: "currency" },
  { key: "total_distance", label: "Total Distance", format: "currency" },
];

export const BOT_DETAILS_RIGHT = [
  { key: "total_steps", label: "Total Steps", format: "number" },
  { key: "per_step_qty", label: "Per Step Qty", format: "number" },
  { key: "rtp_value", label: "RTP Value", format: "number" },
  { key: "total_baskets_one_side", label: "Total Baskets One Side", format: "number" },
  { key: "basket_loss", label: "Basket Loss", format: "currency" },
  { key: "total_baskets", label: "Total Baskets", format: "number" },
  { key: "daily_loss", label: "Daily Loss", format: "currency" },
  { key: "basket_max_qty", label: "Basket Max Qty", format: "number" },
  { key: "upper_limit", label: "Upper Limit", format: "currency" },
  { key: "lower_limit", label: "Lower Limit", format: "currency" },
];

// Header fields (token + datetime) handled separately in the form.
export const HEADER_FIELDS = [
  { key: "exchange", label: "Exchange", format: "text", placeholder: "DERIBIT / HYPERLIQUID" },
  { key: "token_name", label: "Token Name", format: "text", placeholder: "ETH-USD" },
  { key: "token_symbol", label: "Token Symbol", format: "text", placeholder: "Ethereum" },
  { key: "account", label: "Account", format: "text", placeholder: "e.g. Main, Sub-1" },
];

// Every numeric/text column, in DB insert order.
export const ALL_FIELDS = [
  ...HEADER_FIELDS,
  ...METRIC_CARDS,
  WORKING_HOURS_FIELD,
  ...BOT_DETAILS_LEFT,
  ...BOT_DETAILS_RIGHT,
];

// Which exchange an account belongs to. Only used to pre-fill the form for a
// known account — the value is stored per entry, so an account that moves or a
// new venue just needs the field set on the entry itself rather than a code change.
export const EXCHANGE_BY_ACCOUNT = {
  "ETH-HIDDEN":      "DERIBIT",
  "SOL-HIDDEN":      "DERIBIT",
  "BTC-HFT3":        "DERIBIT",
  "SOL-HFT1":        "DERIBIT",
  "HYPER-USMARKETS": "HYPERLIQUID",
};

export const KNOWN_EXCHANGES = ["DERIBIT", "HYPERLIQUID"];

export function formatValue(value, format) {
  if (value === null || value === undefined || value === "") return "—";
  const num = Number(value);
  switch (format) {
    case "currency":
      return `$${num.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
    case "percent":
      return `${num.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}%`;
    case "number":
      return num.toLocaleString(undefined, { maximumFractionDigits: 4 });
    // Volume runs to eight figures, where the exact dollars carry no meaning
    // and the digit count is what you end up counting. Matches the M/K style
    // the Summary Report already uses. Sign is handled separately so a
    // negative reads -$1.20M rather than $-1.20M.
    case "compact-currency": {
      const abs = Math.abs(num);
      const sign = num < 0 ? "-" : "";
      if (abs >= 1_000_000) return `${sign}$${(abs / 1_000_000).toFixed(2)}M`;
      if (abs >= 1_000) return `${sign}$${(abs / 1_000).toFixed(2)}K`;
      return `${sign}$${abs.toFixed(2)}`;
    }
    default:
      return String(value);
  }
}
