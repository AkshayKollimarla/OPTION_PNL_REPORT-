...
// liquid names this book trades). Reported as the instrument's tick, but prices
// are NOT rounded to it: the book records the exact mid, so a 4.75/4.76 market
// fills as 4.755. Replies carry exact_price so the pages skip their own tick
// rounding too.
const TICK = 0.01;

// A mid of two penny quotes is exact to half a cent, but binary floating point
// turns (1.13 + 1.14) / 2 into 1.1349999999999998. Six places strips that
// noise without touching any real digit.
const exact = (v) => Math.round(v * 1e6) / 1e6;

const MONTHS = ["JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC"];

/**
 * Today's date on the US options calendar (New York), as YYYY-MM-DD.
 */
export function usMarketDate(now = new Date()) {
  return now.toLocaleDateString("en-CA", { timeZone: "America/New_York" });
}

/**
 * Has this expiry passed? An option still trades on its expiry day, so it is
 * expired only once that New York date is behind us. Dated by the US market,
 * not UTC or the server's clock: from 8pm New York the UTC date has already
 * moved on, which would retire a contract that is still trading for hours.
 */
export function isExpired(expiryIso, now = new Date()) {
  return Boolean(expiryIso) && String(expiryIso) < usMarketDate(now);
}
...
// The tradable universe changes daily at most — new listings, delistings — so
// it is cached far longer than the chain. 14,000 assets is a 2-second call and
// roughly 4 MB of JSON; fetching it per keystroke would make the symbol picker
// unusable, so it is fetched once and searched in memory.
let assetCache = null;
const ASSET_TTL_MS = 6 * 60 * 60 * 1000;

/**
 * Every US equity that is tradable AND has listed options.
 *
 * The options filter matters: of ~13,400 tradable symbols only ~6,200 carry
 * `has_options`, and offering the rest would let someone pick a symbol whose
 * chain then comes back empty with no explanation.
 */
export async function fetchAssets(key, secret) {
  if (assetCache && Date.now() - assetCache.at < ASSET_TTL_MS) return assetCache.assets;
  const body = await aFetch(
    `${tradeBase(key)}/v2/assets?status=active&asset_class=us_equity`, key, secret
  );
  const assets = (Array.isArray(body) ? body : [])
    .filter((a) => a.tradable && (a.attributes || []).includes("has_options"))
...
/**
 * Symbol search for the picker.
 *
 * Ranked so typing "HO" puts HOOD near the top rather than burying it under
 * every company whose NAME happens to contain "ho": an exact ticker first,
 * then tickers that start with the query, then tickers containing it, and only
 * then a match on the company name.
 */
export async function searchSymbols(query, key, secret, { limit = 60 } = {}) {
  ...
  for (const a of assets) {
    ...
    else if (a.name.toUpperCase().includes(q)) rank = 3;
    else continue;
    scored.push({ rank, a });
    }
  // Within a rank, the shorter ticker first: typing "TSL" wants TSLA ahead of
  // TSLG/TSLL/TSLQ, and typing "PLT" wants PLTR, not PLTD. Length is a decent
  // proxy for the primary listing over its leveraged and structured cousins.
  scored.sort((x, y) =>
    x.rank - y.rank ||
    x.a.symbol.length - y.a.symbol.length ||
    x.a.symbol.localeCompare(y.a.symbol));
...
export async function fetchUnderlying(symbol, key, secret) {
  ...
  // Mid only. With one side of the book empty there is no mid, and the price
  // is reported as missing rather than substituted: the last trade can be
  // hours old and far from where the stock now quotes.
  const mid = bid > 0 && ask > 0 ? exact((bid + ask) / 2) : null;
...
export async function fetchOptionTicker(parts, key, secret) {
  ...
  // Mid only — the price is (bid + ask) / 2 and nothing else. Without both
  // sides there is no price at all: never the last trade or the day's close,
  // which on these contracts routinely sit far from the live market (a HOOD
  // call quoted 0.08/0.11 had last traded at 0.34), and never zero, which a
  // form would take for a real price and write over a saved entry.
  // Alpaca publishes no mark of its own for options, so the mark is the mid.
  const mid = bid > 0 && ask > 0 ? exact((bid + ask) / 2) : null;
  const mark = mid;
  ...
  return {
    tick_size: TICK,
    mark_price_raw: mark,
    // The exact mid, not rounded to the tick.
    mark_price_usd: mark,
    exact_price: true,
    underlying_price: underlying?.mid || null,
    // Alpaca's snapshot carries impliedVolatility (and greeks) on the
    // indicative feed as well as OPRA, as a decimal — 1.2514 is 125.14%. The
    // option pages read mark_iv in percent, as Deribit quotes it, so it is
    // scaled here.
    //
    // It is absent where no volatility can be solved for, typically a strike
    // so deep in the money that the premium is almost all intrinsic. That
    // comes back null rather than zero: the entry form leaves its IV field
    // untouched on null, where a zero would overwrite a typed value.
    mark_iv: Number.isFinite(Number(snapshot?.impliedVolatility)) && Number(snapshot.impliedVolatility) > 0
      ? Number(snapshot.impliedVolatility) * 100
      : null,
