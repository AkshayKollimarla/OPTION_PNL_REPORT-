...
async function _tick() {
  ...
  for (const job of jobs) {
    try {
      ...
      console.error(`[auto-close-combo-worker #${job.id}] unhandled:`, err.message);
      try {
...
async function _closeLegOption(comboJobId, base, token, leg) {
  ...
  if (leg.opt_order_id) {
    ...
    // Still open — re-quote at the current mid if it has moved (chase)
    ...
    if (mid > 0 && Math.abs(mid - orderPrice) > OPT_REQUOTE_THRESHOLD) {
      await _log(comboJobId, `Leg ${leg.leg_index + 1} option mid moved ${orderPrice.toFixed(5)} → ${mid.toFixed(5)}, re-quoting...`);
