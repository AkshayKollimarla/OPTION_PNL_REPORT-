...
async function _createTables() {
  ...
  for (const [col, def] of [
    ...
    // tokens (BTC/ETH) — PnL/target tracking is coin-only, matching the
    // user's explicit request. initial_usdc_equity_usd is purely informational
    // (shown in the entry alert alongside the coin figure), never used in
    // the trigger math.
...
async function _processComboJob(job) {
  ...
  if (job.status === "active") {
    ...
    // Same rule as the single-leg worker — see there for the full rationale.
...
async function _closeLegOption(comboJobId, base, token, leg) {
  ...
  const result  = await placeLimitClose(base, token, leg.opt_instrument, liveSize, leg.opt_dir);
  const orderId = result.order?.order_id;
  const price   = result.order?.price;
  await _log(comboJobId, `Leg ${leg.leg_index + 1} option maker close placed: ${liveSize}x ${leg.opt_instrument} @ ${price} [order ${orderId}]`);
...
async function _closeLegFutures(comboJobId, base, token, leg) {
  ...
  // Unlike options, a futures instrument (e.g. ETH-PERPETUAL) is often
  // shared across multiple legs' hedges — the live position is the
  // COMBINED remaining exposure, not just this leg's share. Cap at the
  // smaller of the stored per-leg qty and the live size: never try to
  // reduce by more than what's actually open (avoids the same
  // invalid_reduce_only_order rejection as the option leg above), and
  // never close more than this leg's own intended hedge while other legs
  // may still need theirs.
  const liveSize = await livePositionSize(base, token, leg.fut_instrument);
  if (liveSize === null) {
    await _log(comboJobId, `Leg ${leg.leg_index + 1}: could not confirm live futures position size for ${leg.fut_instrument} — skipping this tick, will retry.`);
    return;
  }
  if (liveSize === 0) {
    await _log(comboJobId, `Leg ${leg.leg_index + 1} futures position already flat (${leg.fut_instrument}) — nothing left to close.`);
    await pool.query(`UPDATE auto_close_combo_legs SET fut_done=1 WHERE id=?`, [leg.id]);
    return;
  }
  const closeQty = Math.min(Math.abs(futQty), liveSize);

  const result     = await placeMarketClose(base, token, leg.fut_instrument, closeQty, leg.fut_dir);
  const closePrice = parseFloat(result.order?.average_price ?? result.order?.price ?? 0);
  await _log(comboJobId, `Leg ${leg.leg_index + 1} futures market close: ${closeQty}x ${leg.fut_instrument} @ ${closePrice}`);
...
async function _finishComboJob(comboJobId) {
  try {
    ...
    // Same coin-only basis as the trigger check in _processComboJob — Net
    // PnL must be measured against the same number that decided to close.
